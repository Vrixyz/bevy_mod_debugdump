...
pub struct DataGraphContext<'a> {
    settings: &'a Settings,

    data_tracked: HashSet<ComponentId>,
    data_readers: HashMap<ComponentId, Vec<NodeId>>,
    data_writers: HashMap<ComponentId, Vec<NodeId>>,
    schedule: Box<dyn ScheduleLabel>,

    color_edge_idx: AtomicUsize,
}

impl<'a> DataGraphContext<'a> {
    ...
    fn add_data(&self, dot: &mut DotGraph, data: &ComponentId, world: &World) -> String {
        let component = world.components().get_info(*data).unwrap();
        let full_name = component.name();
        // TODO: Relevant name is only what's inside "bevy::ecs::Events<(...)>"
        // let name = full_name.split_once('<').unwrap().1;
        // let name = &name[0..name.len() - 1];
        let name = full_name;
        let event_id = format!("event_{0}", data.index());
...
pub fn data_graph_dot<'a>(
    ...
    for (system_id, system, _condition) in graph.systems() {
        ...
        for access in accesses.reads() {
            ...
            // TODO: we should be able to use most of the same code for events and components/resources
    ...
    DataGraphContext {
        settings,
        data_tracked: events_tracked,
        data_readers: event_readers,
        data_writers: event_writers,
...
pub fn print_only_context(
    ...
    ctx: &DataGraphContext,
    other_ctxs: &[DataGraphContext],
    world: &World,
) {
    ...
    let graph = schedule.graph();
    {
        let all_writers = ctx.data_writers.values().flatten();
        let all_readers = ctx.data_readers.values().flatten();
    ...
    for event in ctx.data_tracked.iter() {
        let readers = ctx.data_readers.get(event).cloned().unwrap_or_default();
        let writers = ctx.data_writers.get(event).cloned().unwrap_or_default();

        let graph_to_write_to = if other_ctxs
            .iter()
            .filter(|c| c.data_tracked.contains(event))
            .count()
            > 1
        {
            &mut *events_dotgraph
        ...
        let event_id = ctx.add_data(graph_to_write_to, event, world);
